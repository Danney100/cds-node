...
app.use((request, response, next) => {
  ...
  if (!authorizationHeader || !authorizationHeader.startsWith("Bearer")) {
    response.set(
      "WWW-Authenticate",
      `Bearer realm="${serviceHost}", error="invalid_token", error_description="No Bearer token provided."`
    );
  ...
  if (!isValid) {
    response.set(
      "WWW-Authenticate",
      `Bearer realm="${serviceHost}", error="invalid_token", error_description="The token is invalid."`
    );
    return response.status(401).end();
  }
    ...
    cards: [
      {
        ...
        summary:
          ...
          " " +
          patientResource.name[0].family[0],
        indicator: "info",
        source: {
          label: "CDS Service Tutorial",
          url:
            "https://github.com/cerner/cds-services-tutorial/wiki/Patient-View-Service",
        },
        links: [
          {
...
app.post("/cds-services/order-select-example", (request, response) => {
  ...
  if (
    ["MedicationRequest", "MedicationOrder"].includes(
      draftOrder.resourceType
    ) &&
    selections.includes(`${draftOrder.resourceType}/${draftOrder.id}`) &&
...
function createMedicationResponseCard(context) {
  const providerOrderedMedication =
    context.medicationCodeableConcept.coding[0].code;
      ...
      cards: [
        {
          ...
          source: {
            label: "CDS Service Tutorial",
            url:
              "https://github.com/cerner/cds-services-tutorial/wiki/Order-Select-Service",
          },
        },
      ],
    };
              ...
              actions: [
                {
                  type: "create",
                  description:
                    "Modifying existing medication order to be Aspirin",
